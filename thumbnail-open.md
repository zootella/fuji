...
**Where a tile sits, at a fractional device pixel ratio.** What the `flowSnap` question became once the Windows box ran it; the measurement is on the site, in the thumbnail pipeline page. Sizing a canvas is solved and about a third of tiles are resampled regardless, because a row whose top lands on a fractional device pixel carries every tile in it off the grid. Two things are left. Whether flooring `flowSnap` rather than rounding closes the half a canvas can reach — indicated by the measurement, resting on two data points, and unmeasured as to what the compositor does with the sliver of box a smaller canvas leaves unfilled. And whether a row can be made to start on the device grid at all, which is the real question and belongs to whoever next touches the sheet's layout rather than to this pipeline.

**175%**, the one Windows scale not yet run. The rule found at 125% and 150% predicts it; nothing has checked.
