...
That second file is the one to keep in mind. Six megabytes of JPEG becomes 6240 × 4160 × 4 bytes once the compression is undone, which is a hundred and four megabytes of raster for one picture. Two hundred of those on screen at once would be twenty gigabytes.
...
### GIF and SVG become an img

A GIF because only an `<img>` animates, and a picture that bounces in a browser should bounce in fuji. Drawing one into a canvas would get its first frame — a perfectly good thumbnail, and what most file managers show — but we would rather show the thing itself. A contact sheet of two hundred animations sounds expensive. It is not: an animation runs only while it is actually in view, so a wall of GIFs costs only the handful on screen.

An SVG because it has no pixels to own. The engine parses it and paints it sharp at whatever size the CSS gives, and an SVG without intrinsic dimensions cannot be drawn into a canvas at all — there is nothing to scale a drawing to.

Shown through an `<img>`, an SVG is an image and not a document: its scripts never run, it loads nothing from outside itself, it receives no events, and it is clipped to the element it is drawn in, so it cannot draw over its neighbours however its coordinates are written. That is why it is safe to show one this way, and why we never show one inline in fuji's own document, where its scripts would run as fuji.
...
Before any thumbnail is made, one Rust call, *the probe*, reads the file's first bytes:

```js
thumbnailProbe(paths)   // → [{format, width, height, problem}, …]
```

For each file path it answers what the bytes say the file is, how big the header claims the picture is, and, if fuji will not show it, why. Nothing is decoded to answer this.
...
A tile whose aspect ratio is known can take its final size before its pixels arrive, so the thumbnail lands in a box that is already there. Without the sizes the contact sheet reflows on every arrival, each picture nudging the ones after it down the page as it appears.
...
On macOS nothing takes this route at all: every raster extension fuji opens is on that platform's native list. It is reachable there only by emptying the list, which is how it gets tested.
...
The longer side goes to the box, and nothing is ever enlarged:

```js
let scale = Math.min(box / size.x, box / size.y, 1)
```

A square picture lands exactly on the box, a wide one hits the limit on width alone, and a picture already smaller than the box keeps its own size rather than being blown up. **Nothing is cropped** — every picture is shown whole, and the rows come out ragged because of it. The box is one of four named sizes: Small, Medium, Large and XL, at 120, 240, 360 and 480 CSS pixels. All four are settings, so Medium means whatever the user says it means.
...
**The page tags the pixels with what the header said**, so Windows' always-sRGB pixels land correctly on a wide-gamut canvas rather than being reinterpreted as wide values.

Measured with three PNGs authored so that *Core Graphics* performed every conversion and no value was typed in by hand. Each halved image has a primary that lies outside sRGB on the left, and the *sRGB* primary expressed in P3 coordinates on the right — the same physical color, one in gamut and one out. On a correct pipeline the halves differ visibly; if anything clamps, they become identical.
...
The control runs the other way, and it matters as much: an sRGB file must be *converted* into P3 coordinates, not reinterpreted as P3, or every ordinary image on the contact sheet would come out oversaturated. It converts correctly both ways.
...
- **Animated WebP** is a still on the native route. Sending it to an `<img>` the way a GIF goes means reading the animation flag out of its header in the probe.
...
- **`flowSnap` at a fractional device pixel ratio**, per the Windows note above. Until it is measured, the most we claim for Windows is "no worse than before".
- **WIC's color behaviour** is read from its documentation and has never been run against a file with a profile in it.
- **A change of monitor.** Canvases are never remade when a window moves to a different screen, so the device pixel ratio and the gamut both go stale until the contact sheet rebuilds.
