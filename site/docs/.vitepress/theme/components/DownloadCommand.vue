<script setup>//./.vitepress/theme/components/DownloadCommand.vue
import { ref, onMounted } from 'vue'
import { fetchSidecar } from '../downloads.js'

/*
One install command with the current hash written into it. The command itself is an ordinary fenced code block on the download page, highlighted during the build like every other fence, and this component wraps it:
...
A fence alone would name the hash of whatever was published the day the site was built, and the page's whole arrangement is that publishing an installer changes what the site says without a rebuild.
*/

let props = defineProps({
	file: String,//the published name, fuji.dmg or fuji.exe; its sidecar sits beside it under the same name
})
...
onMounted(async () => {
	...
	loaded.value = true
	if (!sidecar.value) return//unpublished: the zeros stay, the line below says so, and a pasted command refuses to install, which is the right refusal
