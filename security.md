...
2. **The first bytes decide the format, before any decoder sees the file.** `canvas.md` has the signatures. A file whose bytes are not a format fuji handles is refused, whatever its extension. This turns "any file ImageIO or WIC will parse", sixty formats on the Mac, into the handful fuji actually shows.
3. **A size ceiling from the header, before any decode allocates.** The decompression bomb is the attack that needs no bug.
...
The page's paths are sandboxed and the IPC is not scoped. `thumbnail.rs` exists, is unsandboxed, decodes whatever the library will take, has no size ceiling, and is called by nothing. The content security policy in `tauri.conf.json` allows only fuji's own scripts and its ipc, and blob and data urls for images. None of the walls above is built; this file is the order to build them in.
