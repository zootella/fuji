...
function flowSnap(side, have) {//how many pixels a canvas gets for one axis: the css box in device pixels, or the pixels in hand where those cannot reach it
	...
	This rule is whole on a mac and only half the story on windows, and the half it does not cover is not a canvas's to fix. Windows scales at 125, 150 and 175 percent, so the ratio is fractional and a box on whole css pixels lands on a fraction of a device pixel however this rounds. Measured at two of those scales: what this function computes is right every time, and about a third of tiles are resampled anyway, because row heights accumulate in halves and quarters and a row whose top misses the device grid takes every tile in it along. Sizing a canvas cannot reach a row's position. The thumbnail pipeline document on fuji's site has the twenty tiles and the rule they fit, including the one change indicated here and not yet made: this rounds up, and every clean tile that differed from its box was smaller than it.
	*/
