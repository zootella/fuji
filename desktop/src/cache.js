...
There is one store per window rather than one per fuji, which matters on macOS where a single process holds several. Each window is its own webview with its own heap, so this module exists once per window and two windows on the same folder decode everything twice — the page cache makes the second read free, and nothing makes the second decode free.
