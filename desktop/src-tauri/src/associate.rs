...
**Which is why `bundle.fileAssociations` is absent from tauri.conf.json and must stay absent.** It is Tauri's own feature for this and it looks like the obvious thing to turn on. On macOS it would be fine; on Windows it inserts that NSIS macro, which takes each extension's default value at install time, silently, without asking — the one thing this whole design refuses to do. So the macOS half is a hand-written Info.plist that Tauri merges, and the Windows half is this module at runtime. Everything here makes fuji available. Nothing here makes fuji default, because since Windows 8 the default lives in a hash-protected key that only the user, through the system's own interface, can set.
