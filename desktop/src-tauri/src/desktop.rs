...
The reason it lives down here is that the page cannot do it. RunEvent::Exit is the one event every quit path reaches: closing the last window raises RunEvent::ExitRequested first, but that never fires for the macOS Quit menu item, the Dock's Quit, or a logout, which all send terminate: straight to the application; tao turns that into applicationWillTerminate:, then LoopDestroyed, which arrives here as Exit.

**lib.rs writes from both events, and the earlier one is not an optimisation.** On macOS fuji stays running after its last window closes, so Exit may be hours away or may never arrive before the machine is turned off — a write held for it would simply be lost. ExitRequested is the moment the last window goes, on every platform, and on Windows and Linux it is immediately followed by Exit. The map drains as it writes, so the second call finds nothing and costs nothing. The webview is in fact still alive at that moment — what is gone is the opportunity, because Exit arrives on the main thread inside an operating system callback. Synchronous work like a file write is fine, but a round trip to JavaScript needs the main thread's run loop to turn, and it will not turn again before the process is gone.
