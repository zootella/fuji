...
**That audit has now been run here, on 2026-09-13 at 125% and 150%, and it is written up on the site rather than in this file** — the thumbnail pipeline page owns it, because the answer turned out to be about the sheet's layout rather than about pixel units. The short form, so this section is not misleading on its own: `flowSnap` computes correctly at both fractional ratios, and roughly a third of tiles are resampled regardless, because a row whose top lands on a fractional device pixel carries every tile in it off the grid. Sizing a canvas cannot reach that. The question this section left open — whether fuji can predict which way Chromium snaps a box — is answered for the canvas and reopened one level up, at the row.
