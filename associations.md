...
**The uninstall hook, Windows only.** Fuji writes its association keys at runtime and the uninstaller knows nothing about them. Measured on the Windows 10 box: an uninstall takes the install folder, both shortcuts, the uninstall key and the WebView2 cache, and leaves all ten `Fuji.*` ProgIDs, the `Capabilities` block, the `RegisteredApplications` value and all ten `OpenWithProgids` entries — so Windows goes on believing fuji opens `.webp`, through a command naming an executable that is gone. `NSIS_HOOK_PREUNINSTALL` is where those get deleted, and only the keys fuji created. No install hook. macOS needs nothing, because trashing the `.app` takes its claims with it.
...
**The uninstaller's *delete the application data* checkbox reaches the WebView2 cache and not fuji's own state.** It removes `%APPDATA%\<identifier>` and `%LOCALAPPDATA%\<identifier>`, where the web engine keeps a cache that runs to a hundred megabytes and is genuinely the bulk of what fuji leaves on a disk. `fuji.toml` and the `fuji-temp` logs sit in the home folder and survive it — deliberate, since a settings file a user edited is not obviously the installer's to destroy, but it means that checkbox is not the whole story.
