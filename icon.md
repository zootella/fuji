...
**A light Start menu keeps the tile black, and that is accepted.** Since the October 2020 update, Windows 10 draws its own tile behind Chrome, Edge and Firefox — milky white in light mode, dark in dark — whatever color their manifests name, and it does not do the same for fuji. Tried 2026-09-25 on a clean Windows 10 Pro 21H2 VM in light mode, with Edge pinned beside fuji as the control: first the named `gray`, then Chrome's manifest copied attribute for attribute, `#5F6368` and `Square44x44Logo` included. Each was drawn as written, dark beside Edge's white. So whatever earns the browsers the theme's tile is outside the manifest, and the untested candidates are the logo image and fuji's per-user install under `AppData`, where the browsers install to `Program Files`. It was dropped rather than chased, because it shows only on Windows 10 with Start switched to light, a fresh Windows 10 ships it dark, and black is right there. `transparent`, the value that sounds like the answer, is not allowed: the desktop schema takes `#RRGGBB` or sixteen named colors, and an invalid value makes Windows ignore the whole file.

**Seen, and it works.** The user looked at the medium tile on this machine on 2026-09-11 and judged it right at the first radius — a cyan disc at two-thirds of a black square, the name beneath it. No adjustment was called for, so 338 stands where the macOS disc needed two attempts to reach 446.
...
**What remains unestablished** is only ordering. Microsoft's instructions say the manifest must be in place *before* the shortcut is created, and that an existing shortcut must be nudged — touching its `lastwritetime` — for a changed manifest to be picked up. A first install should be fine, since installers write their files before creating shortcuts, but that was inferred rather than watched. Seen 2026-09-25: touching the shortcut does make Windows read the manifest again, and event 28032 logs each read; and a reinstall rewrites the shortcut and puts the pin back where it was, because the Start menu remembers a tile by the shortcut's app ID, `app.fujidesktop.Fuji`, which is `identifier` in `tauri.conf.json`. **An upgrade over an existing install may keep a stale tile**, and fixing that would need an NSIS hook, which is the machinery this was meant to avoid. The symptom would be a default tile after an upgrade, and the workaround is one `lastwritetime` touch.
