...
- **`menu.md`** — the macOS menu bar and the Dock icon's right-click menu, a macOS-only subject because a menu belongs along the top of the screen there and inside the window everywhere else. All of it is built: New Window, an Open… that behaves exactly like a dragged-in file, fuji's own Toggle Full Screen beside the system's rather than instead of it, the window list the menu bar needed registering to get, and one New Window item on the Dock icon. Says what each is for and why, keeps the Edit menu's stubs deliberately, and records Open Recent as decided against — useless for thousands of pictures, and a menu that names private files.
