...
- **`security.md`** — how fuji safely decodes pictures it did not make. Where untrusted bytes are parsed, why the web engine's sandbox is not protecting fuji as things stand, and the walls to build, in order. Also what fuji thinks a file *is*: the two ways it answers that, by extension and by first bytes, and the open case of a picture arriving with no extension at all, which fuji currently cannot see.
- **`associations.md`** — becoming an application the operating system will hand a picture to. The first pass is built and verified on both platforms, with its reasoning in `associate.rs` and `Info.plist`; what is left here is the concession that nobody will find how to use it, what a user actually meets on each platform, the uninstall hook still to build, the File Extensions settings page planned for the second pass, and the platform facts worth not researching twice.
...
`scripts.js` at the root is the build pipeline both workspaces call into, by a verb. Its own opening essay says why it is one file rather than one per workspace; `CLAUDE.md` lists the commands that reach it.
